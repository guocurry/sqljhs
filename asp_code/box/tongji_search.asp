...
<style>
.selectpicker .btn span:first-child{overflow: hidden;
white-space: nowrap;
...
<div id="bts-ex-4" class="selectpicker" data-live="true" data-clear="true">
                        <a href="#" class="clear"><span class="fa fa-times"></span><span class="sr-only">Cancel the selection</span></a>
							...
							<span class="placeholder">选择主体</span>
								...
								<div class="live-filtering" data-clear="true" data-autocomplete="true" data-keys="true">
									...
									<div class="search-box">
										<div class="input-group">
											...
											<input type="text" placeholder="搜索主体" id="input-bts-ex-4" class="form-control live-search" aria-describedby="search-icon3" tabindex="1" />
										</div>
									</div>
...
while not rs.eof%>
											<li onClick="zhuti_(<%=rs(0)%>)" class="filter-item items" data-filter="<%=rs(1)%>" data-value="<%=rs(0)%>"><%=rs(1)%></li>
<%rs.movenext
wend
end if
rs.close
set rs=nothing%>
							...
							<input type="hidden" id="zhuti_num" name="bts-ex-4" value="0">
  </div>
</div>
                        <div class="tj_search_box">
                        <h2>属性数据</h2>
                        <div id="bts-ex-5" class="selectpicker" data-live="true" data-clear="true">
                        <a href="#" class="clear"><span class="fa fa-times"></span><span class="sr-only">Cancel the selection</span></a>
							<button data-id="prov" type="button" class="btn btn-lg btn-block btn-default dropdown-toggle">
							<span class="placeholder">选择属性</span>
							<span class="caret"></span>
							</button>
							<div class="dropdown-menu">
								<div class="live-filtering" data-clear="true" data-autocomplete="true" data-keys="true">
									...
									<div class="search-box">
										<div class="input-group">
											...
											<input type="text" placeholder="搜索" id="input-bts-ex-5" class="form-control live-search" aria-describedby="search-icon3" tabindex="1" />
										</div>
									</div>
									<div class="list-to-filter">
										<ul class="list-unstyled">
...
if not rs.eof and not rs.bof then
while not rs.eof%>
											<li onClick="shuxing_(<%=rs(0)%>)" class="filter-item items" data-filter="<%=rs(1)%>" data-value="<%=rs(0)%>"><%=rs(1)%></li>
<%rs.movenext
wend
end if
rs.close
set rs=nothing%>
							...
							<input type="hidden" id="shuxing_num" name="bts-ex-5" value="1">
						</div>
                        </div>
                         <div class="tj_search_box">
                         <h2>开始年份</h2>
                         <div id="bts-ex-3" class="selectpicker" data-clear="true">
							<a href="#" class="clear"><span class="fa fa-times"></span><span class="sr-only">Cancel the selection</span></a>
							<button data-id="prov" type="button" class="btn btn-lg btn-block btn-default dropdown-toggle">
							<span class="placeholder">2011</span>
							<span class="caret"></span>
							</button>
							<div class="dropdown-menu">
								<ul class="list-unstyled" style="max-height:300px; overflow:hidden; overflow-y:auto;">
                                <%for i=1991 to year(date())%>
									<li onClick="kaishi_(<%=i%>)" class="items<%if i=2011 then%> selected<%end if%>" data-value="<%=i%>"><%=i%></li>
	<%next%>
								</ul>
							</div>
							<input type="hidden" name="bts-ex-3" id="kaishi_num" value="2011">
						</div>
                        </div>
                         <div class="tj_search_box">
                         <h2>结束年份</h2>
                         <div id="bts-ex-2" class="selectpicker" data-clear="true">
							<a href="#" class="clear"><span class="fa fa-times"></span><span class="sr-only">Cancel the selection</span></a>
							<button data-id="prov" type="button" class="btn btn-lg btn-block btn-default dropdown-toggle">
							<span class="placeholder"><%=year(date())%></span>
							<span class="caret"></span>
							</button>
							<div class="dropdown-menu">
								<ul class="list-unstyled" style="max-height:300px; overflow:hidden; overflow-y:auto;" id="jiesu_year">
                                <%for i=1991 to year(date())%>
									<li onClick="jieshu_(<%=i%>)" class="items<%if i=2016 then%> selected<%end if%>" data-value="<%=i%>"><%=i%></li>
	<%next%>
								</ul>
							</div>
							<input type="hidden" name="bts-ex-2" id="jieshu_num" value="<%=year(date())%>">
						</div>
                        </div>
<script>
...
function class_(a){
	...
	/////////
	$.post("/box/tongji_mains.asp",{zhuti:zhuti,shuxing:shuxing,kaishi:kaishi,jieshu:jieshu,tjclass:a},
...
function zhuti_(a){
	...
	var tjclass=$("input[name=tongjiclass]:checked").val();
	/////////
	$.post("/box/tongji_mains.asp",{zhuti:a,shuxing:shuxing,kaishi:kaishi,jieshu:jieshu,tjclass:tjclass},
   function(data,status){
     if(status=="success"){
		 $("#tj_sbox").html(data);
		 }else{$("#tj_sbox").html("加载失败，请稍后重试");}
...
function shuxing_(a){
	...
	$.post(shuxingget,{zhuti:zhuti,shuxing:a,kaishi:kaishi,jieshu:jieshu,tjclass:tjclass},
   function(data,status){
     if(status=="success"){
		 $("#tj_sbox").html(data);
		 }else{$("#tj_sbox").html("加载失败，请稍后重试");}
...
function kaishi_(a){
	...
	var tjclass=$("input[name=tongjiclass]:checked").val();
	/////////
	$.post("/box/tongji_mains.asp",{zhuti:zhuti,shuxing:shuxing,kaishi:a,jieshu:jieshu,tjclass:tjclass},
   function(data,status){
     if(status=="success"){
		 $("#tj_sbox").html(data);
		 }else{$("#tj_sbox").html("加载失败，请稍后重试");}
...
function jieshu_(a){
	...
	$.post("/box/tongji_mains.asp",{zhuti:zhuti,shuxing:shuxing,kaishi:kaishi,jieshu:a,tjclass:tjclass},
   function(data,status){
     if(status=="success"){
		 $("#tj_sbox").html(data);
		 }else{$("#tj_sbox").html("加载失败，请稍后重试");}
