...
rvs.open "select * from big_proclass ",conn,1,1
...
rs.open "select * from 68_proclass where id="&request.QueryString("id"),conn,1,1
...
if not rvs.eof and not rvs.bof then
while not rvs.eof%>
          <option value="<%=rvs(0)%>"<%if trim(rs("bid"))=trim(rvs(0)) then%> selected="selected"<%end if%>><%=rvs(1)%></option>
<%rvs.movenext
wend
end if
rvs.close
...
set rs=server.createobject("adodb.recordset")
rs.open "select * from 68_proclass where id="&request.QueryString("id"),conn,1,3
if not rs.eof then
	rs("title")=request.Form("tt")
	rs("bid")=request.Form("class")
rs.update
Response.Write("<script>alert('修改成功');window.location.href='smallclass_add.asp?id="&request.QueryString("id")&"';</script>")
else
...
rsv.open "select * from 68_proclass ",conn,1,3
rsv.addnew
...
$(document).ready(function(){
  ...
  $(".sure").click(function(){
  $(".tip").fadeOut(100);
  window.location.href="smallclass_add.asp?class=<%=request.Form("class")%>";
});
});
</script>
<%conn.close
