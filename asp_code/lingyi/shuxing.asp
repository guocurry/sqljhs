...
$(document).ready(function(){
 ...
 window.location.href='shuxing_add.asp'
  });
...
rs.open "select * from shuxing order by id desc ",conn,1,1
...
n=n+1%>
        ...
        <td><%=rs(1)%></td>
        <td><%=rs(2)%></td>
        <td><a href="shuxing_add.asp?id=<%=rs(0)%>" class="tablelink">修改</a></td>
        </tr> 
<%rs.movenext 
i=i+1 
j=j-1
...
dim sql
sql="delete from [shuxing] where id in ("&Request.Form("id")&")"
conn.Execute ( sql )
Response.Write("<script>alert(""删除成功！"");javascript:window.location.href='shuxing.asp';</script>")
else
