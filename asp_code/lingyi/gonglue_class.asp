...
$(document).ready(function(){
 ...
 window.location.href='gonglue_class_add.asp'
  });
...
rs.open "select * from news_s_class order by id desc ",conn,1,1
...
n=n+1%>
        ...
        <td><%=rs(0)%></td>
        <td><span class="yasnecolor" style="background-color:<%=rs("yanse")%>"><%=rs(1)%></span></td>
        <td><a href="gonglue_class_add.asp?id=<%=rs(0)%>" class="tablelink">修改</a></td>
        </tr> 
<%rs.movenext 
i=i+1 
j=j-1
...
dim sql
sql="delete from [news_s_class] where id in ("&Request.Form("id")&")"
conn.Execute ( sql )
Response.Write("<script>alert(""删除成功！"");javascript:window.location.href='gonglue_class.asp';</script>")
else
