...
$(document).ready(function(){
 ...
 window.location.href='car_chexi_add.asp'
  });
...
<div class="place">
    ...
    <div class="tools">
        ...
        <ul class="toolbar1">
        搜索用户：<input name="" id="suser" type="text" class="scinput" style="margin-right:10px;" /><input name="" type="button" class="scbtn" value="查询" id="searchbtn"/>
        </ul>
...
rs.open "select * from lj_huiyuan where zhanghao like '%"&request.QueryString("u")&"%' order by id desc",conn,1,1
else
rs.open "select * from lj_huiyuan order by id desc",conn,1,1
end if
...
n=n+1%>
        ...
        <td><%=rs(0)%></td>
        <td><%=rs("zhanghao")%></td>
        <td><%=rs("dianhua")%><p style="padding:0px; margin:0px; color:#666;"><%=rs("youxiang")%></p></td>
        <td><%=rs("danwei")%></td>
        <td><%=rs("xingming")%></td>
        <td><%=rs("addtime")%><p style="padding:0px; margin:0px; color:#666;"><%=rs("logintime")%></p></td>
        <td><a href="reg_user.asp?id=<%=rs(0)%>" class="tablelink">修改</a></td>
        </tr> 
<%rs.movenext 
i=i+1 
j=j-1
...
dim sql
sql="delete from [lj_huiyuan] where id in ("&Request.Form("id")&")"
conn.Execute ( sql )
Response.Write("<script>alert(""删除成功！"");javascript:window.location.href='user.asp';</script>")
else
...
<script type="text/javascript">
  ...
  $("#searchbtn").click(function(){
	  if($("#suser").val()!=""){
		  window.location.href='user.asp?u='+$("#suser").val();
		  }else{alert('请输入搜索关键词');}
	  })
	</script>    
    <script type="text/javascript">
