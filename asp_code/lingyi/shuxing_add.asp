...
<div class="formbody">
 <%if request.QueryString("id")="" or not isnumeric(request.QueryString("id")) then%>   
  ...
  <ul class="forminfo">
    <li><label>属性名称</label><input name="tt" type="text" id="tt" class="dfinput" /></li>
	<li><label>属性单位</label><input name="danwei" type="text" id="danwei" class="dfinput" /></li>
...
rs.open "select * from shuxing where id="&request.QueryString("id"),conn,1,1
if not rs.eof and not rs.bof then%>
<div class="formtitle"><span>修改信息</span></div>
    <form id="form1" name="form1" method="post" action="?act=regok&id=<%=rs(0)%>">
  <ul class="forminfo">
    <li><label>属性名称</label><input name="tt" value="<%=rs(1)%>" type="text" id="tt" class="dfinput" /></li>
    <li><label>属性单位</label><input name="danwei" type="text" id="danwei" value="<%=rs(2)%>" class="dfinput" /></li>
    <li><label>&nbsp;</label><input name="" id="addbut" type="button" class="btn" value="确认保存"/>
...
<%if request.QueryString("act")="regok" and request.Form("tt")<>"" and request.QueryString("id")<>"" and isnumeric(request.QueryString("id")) and request.Form("danwei")<>"" then
set rs=server.createobject("adodb.recordset")
rs.open "select * from shuxing where id="&request.QueryString("id"),conn,1,3
if not rs.eof and not rs.bof then
rs(1)=request.Form("tt")
rs(2)=request.Form("danwei")
rs.update
...
%>
<%if request.QueryString("act")="addok" and request.Form("tt")<>"" and request.Form("danwei")<>""  then
set rs=server.createobject("adodb.recordset")
rs.open "select * from shuxing",conn,1,3
rs.addnew
rs(1)=request.Form("tt")
rs(2)=request.Form("danwei")
rs.update
...
$(document).ready(function(){
  ...
  $(".tip").fadeOut(100);
  window.location.href="shuxing.asp";
});
