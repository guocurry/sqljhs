...
$(document).ready(function(){
  ...
  window.location.href='smallclass_add.asp'
  });
...
rs.open "select * from 68_proclass order by id desc",conn,1,1
...
%>
    ...
    <td class="imgtd"><%=rs("title")%></td>
    <td><%set rss=server.createobject("adodb.recordset")
rss.open "select * from big_proclass where id ="&rs("bid"),conn,1,1
if not rss.eof and not rss.bof then
response.Write rss("title")
end if
rss.close
set rss=nothing%></td>
    <td><a href="smallclass_add.asp?id=<%=rs(0)%>"><img src="images/t02.png" /></a></td>
    </tr>
<%rs.movenext 
i=i+1 
j=j-1
...
dim sql
sql="delete from [68_proclass] where id in ("&Request.Form("id")&")"
conn.Execute ( sql )
Response.Write("<script>alert(""删除成功！"");javascript:window.location.href='smallclass.asp';</script>")
else
