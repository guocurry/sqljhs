...
$(document).ready(function(){
 ...
 window.location.href='page_add.asp'
  });
...
rs.open "select * from page order by page_id desc",conn,1,1
...
n=n+1%>
        ...
        <td><%=rs(0)%></td>
        <td><%=rs(1)%></td>
        <td><%set rvs=server.createobject("adodb.recordset")
rvs.open "select * from page_class where id="&rs("bid"),conn,1,1
if not rvs.eof and not rvs.bof then
response.Write rvs(1)
end if
rvs.close
set rvs=nothing%></td>
        <td><a href="page_add.asp?id=<%=rs(0)%>" class="tablelink">修改</a></td>
        </tr> 
<%rs.movenext 
i=i+1 
j=j-1
...
dim sql
sql="delete from [page] where page_id in ("&Request.Form("id")&")"
conn.Execute ( sql )
Response.Write("<script>alert(""删除成功！"");javascript:window.location.href='page.asp';</script>")
else
