...
$(document).ready(function(){
 ...
 window.location.href='tongji_add.asp'
  });
...
rs.open "select * from statistical order by id desc",conn,1,1
...
n=listnum*nn
    ...
    <table class="tablelist">
    	<thead>
    	<tr>
        <th width="8%"><input name="chkall" type="checkbox" id="chkall" value="select" onclick="CheckAll(this.form)" style="border:0" /><label for="chkall">全选</label></th>
...
rss.open "select * from zhuti where id="&rs("zhuti"),conn,1,1
...
set rss=nothing%></td>
        <td><% set rss=server.createobject("adodb.recordset")
rss.open "select * from shuxing where id="&rs("shuxing"),conn,1,1
if not rss.eof and not rss.bof then
danwei="("&rss("danwei")&")"%>
<a href="#"><%=rss(1)%> (<%=rss("danwei")%>)</a>
<%end if
rss.close
set rss=nothing%></td>
        <td><%=rs("nianfen")%></td>
        <td><%=rs("info")%><%=danwei%></td>
        <td><a href="tongji_add.asp?id=<%=rs(0)%>" class="tablelink">修改</a></td>
        </tr> 
<%rs.movenext 
i=i+1 
j=j-1
...
dim sql
sql="delete from [statistical] where id in ("&Request.Form("id")&")"
conn.Execute ( sql )
Response.Write("<script>alert(""删除成功！"");javascript:window.location.href='tongji.asp';</script>")
else
