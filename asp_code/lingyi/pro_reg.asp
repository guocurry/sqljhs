...
rs.open "select * from map where id="&request.QueryString("id"),conn,1,1
...
<div class="formtitle"><span>修改信息</span></div>
  ...
  <li>
      <label>数据类型</label><div class="vocation"><select name="class" onchange="showclass(this.value)" class="select2">
          <option value="0"<%if rs("class")=0 then%> selected="selected"<%end if%>>垃圾设施地图数据</option>
          <option value="1"<%if rs("class")=1 then%> selected="selected"<%end if%>>垃圾监管设施数据</option>
      </select></div></li>
      <li>
  <li><label>设施名称</label><input name="ssmc" type="text" id="ssmc" class="dfinput" value="<%=rs("ssmc")%>"/></li>
     <li>
      <label>设施类型</label><div class="vocation"><select name="sslx" onchange="showclass(this.value)" class="select2">
          <option value="卫生填埋"<%if rs("sslx")="卫生填埋" then%> selected="selected"<%end if%>>卫生填埋</option>
          <option value="简易填埋"<%if rs("sslx")="简易填埋" then%> selected="selected"<%end if%>>简易填埋</option>
          <option value="焚烧"<%if rs("sslx")="焚烧" then%> selected="selected"<%end if%>>焚烧</option>
          <option value="好氧生物处理"<%if rs("sslx")="好氧生物处理" then%> selected="selected"<%end if%>>好氧生物处理</option>
          <option value="厌氧生物处理"<%if rs("sslx")="厌氧生物处理" then%> selected="selected"<%end if%>>厌氧生物处理</option>
          <option value="资源再生"<%if rs("sslx")="资源再生" then%> selected="selected"<%end if%>>资源再生</option>
          <option value="其它"<%if rs("sslx")="其它" then%> selected="selected"<%end if%>>其它</option>
      </select></div></li>
      <li>
      <label><font color="#FF0000">*设施技术路线</font></label><div class="vocation"><select name="ssjslx" onchange="showclass(this.value)" class="select2">
          <option value="反应器型好氧生物处理"<%if rs("ssjslx")="反应器型好氧生物处理" then%> selected="selected"<%end if%>>反应器型好氧生物处理</option>
          <option value="太阳能生物处理"<%if rs("ssjslx")="太阳能生物处理" then%> selected="selected"<%end if%>>太阳能生物处理</option>
          <option value="其它">其它</option>
      </select></div></li><input type="hidden" id="map_in" class="map" name="map" value="<%=rs("map")%>">
	<li><label>地理坐标</label><input name="maps" type="text"  class="dfinput" id="map_num" value="<%=rs("map")%>" readonly="readonly"/> <a href="javascript:;" id="showmapbtn" onclick="show_map()">请在地图上选择标记</a>
    <a href="javascript:;" id="hidemapbtn" style="display:none;" onclick="hide_map()">关闭地图显示</a></li>
    <div id="mapshow" style="display:none;">map</div> 
       <li><label>设施所在城市</label><input name="ssszcs" type="text" id="tt" class="dfinput" value="<%=rs("sszzcs")%>"/></li>
       <li><label>设施所在的区</label><input name="ssszq" type="text" id="tt" class="dfinput" value="<%=rs("ssszq")%>"/></li>
       <li><label>管理部门</label><input name="glbm" type="text" id="tt" class="dfinput" value="<%=rs("glbm")%>"/></li>
       <li><label>运营商</label><input name="yys" type="text" id="tt" class="dfinput" value="<%=rs("yys")%>"/></li>
       <li><label><font color="#FF0000">*运营商电话</font></label><input name="yysdh" type="text" id="tt" class="dfinput" value="<%=rs("yysdh")%>"/></li>
       <li><label>处理能力</label><input name="clnl" type="text" id="tt" class="dfinput" value="<%=rs("clnl")%>"/></li>
       <li><label>建成年份</label><input name="jcnf" type="text" id="tt" class="dfinput" value="<%=rs("jcnf")%>"/></li>
       <li><label>使用年限</label><input name="synx" type="text" id="tt" class="dfinput" value="<%=rs("synx")%>"/></li>
       <li><label>占地面积</label><input name="zdmj" type="text" id="tt" class="dfinput" value="<%=rs("zdmj")%>"/></li>      
      <li>
      <label>照片</label><input name="zp" type="text" class="dfinput" id="img" value="<%=rs("zp")%>" size="70">              
<input type="button" id="image" class="btn" value="选择图片" /></li>
 <li>
      <label>卫星影像</label><input name="wxyx" type="text" class="dfinput" id="imgs" value="<%=rs("wxyx")%>" size="70">              
<input type="button" id="images" class="btn" value="选择图片" /></li>
<li><label><font color="#FF0000">*管理员姓名</font></label><input name="glyxm" type="text" id="tt" class="dfinput" value="<%=rs("glyxm")%>"/></li> 
<li><label><font color="#FF0000">*管理员电话</font></label><input name="glydh" type="text" id="tt" class="dfinput" value="<%=rs("glydh")%>"/></li> 
<li><label><font color="#FF0000">*处理量</font></label><input name="cll" type="text" id="tt" class="dfinput" value="<%=rs("cll")%>"/></li> 
 <li>
      <label><font color="#FF0000">*运行情况</font></label><div class="vocation"><select name="yxqk" onchange="showclass(this.value)" class="select2">
          <option value="正常运行"<%if rs("yxqk")="正常运行" then%> selected="selected"<%end if%>>正常运行</option>
          <option value="设备故障"<%if rs("yxqk")="设备故障" then%> selected="selected"<%end if%>>设备故障</option>
          <option value="暂停运行"<%if rs("yxqk")="暂停运行" then%> selected="selected"<%end if%>>暂停运行</option>
      </select></div></li>
<li><label><font color="#FF0000">*监控摄像头</font></label><input name="jksxtxh" type="text" id="tt" class="dfinput" value="<%=rs("jksxtxh")%>"/></li>
...
set rs=server.createobject("adodb.recordset")
rs.open "select * from map where id="&request.QueryString("id"),conn,1,3
if not rs.eof then
rs("class")=request.Form("class")
rs("ssmc")=request.Form("ssmc")
rs("sslx")=request.Form("sslx")
rs("ssjslx")=request.Form("ssjslx")
rs("map")=request.Form("map")
rs("sszzcs")=request.Form("ssszcs")

rs("ssszq")=request.Form("ssszq")
rs("glbm")=request.Form("glbm")
rs("yys")=request.Form("yys")
rs("yysdh")=request.Form("yysdh")
rs("clnl")=request.Form("clnl")
rs("jcnf")=request.Form("jcnf")
rs("synx")=request.Form("synx")
rs("zdmj")=request.Form("zdmj")
rs("zp")=request.Form("zp")
rs("wxyx")=request.Form("wxyx")
rs("glyxm")=request.Form("glyxm")
rs("glydh")=request.Form("glydh")
rs("cll")=request.Form("cll")
rs("yxqk")=request.Form("yxqk")
rs("jksxtxh")=request.Form("jksxtxh")
rs.update
Response.Write("<script>alert('修改成功');window.location.href='pro_reg.asp?id="&request.QueryString("id")&"';</script>")
else
