...
rs.open "select * from fzjc order by id desc",conn,1,1
...
%>
    ...
    <td class="imgtd"><%=rs(1)%></td>
    <td class="imgtd"><%=rs(2)%> - <%=rs(3)%></td>
    <td><%=rs("sqrk")%>/<%=rs("sqmj")%></td>
    <td><a href="fzjc_reg.asp?id=<%=rs(0)%>">查看</a></td>
    </tr>
<%rs.movenext 
i=i+1 
j=j-1
...
dim sql
sql="delete from [fzjc] where id in ("&Request.Form("id")&")"
conn.Execute ( sql )
Response.Write("<script>alert(""删除成功！"");javascript:window.location.href='fzjc.asp';</script>")
else
